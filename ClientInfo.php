<?php
	/**
    * Class for getting visitor info purpose
    */
	
	class ClientInfo
	{
		private $_continents = array(
				"AF" => "Africa",
				"AN" => "Antarctica",
				"AS" => "Asia",
				"EU" => "Europe",
		...
		*/
		...
		{
			$client  = @$this->_serv['HTTP_CLIENT_IP'];
			$forward = @$this->_serv['HTTP_X_FORWARDED_FOR'];
			$remote  = $this->_serv['REMOTE_ADDR'];
		...
		/**
		* Get geo info about visitor
		* @return array GEO
		*/
		function getVisitorGeo() 
		{
			$output = NULL;
			...
			if (filter_var($ip, FILTER_VALIDATE_IP)) 
			{
				...
				if (@strlen(trim($ipdat->geoplugin_countryCode)) == 2)
				{
					...
					$output = array(
						...
						"continent"      => @$this->_continents[strtoupper($ipdat->geoplugin_continentCode)],
		...
		function checkPlatformVersion()
		{
			foreach ($oses as $os => $pattern)
				if (preg_match('/' . $pattern . '/i', $this->_agent))
					return $os;
			return self::OPERATING_SYSTEM_UNKNOWN;
		}
